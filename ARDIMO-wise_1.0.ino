    ...
    ProgValue=Serial.readString();
      if (ProgValue=="Y" or ProgValue=="y") {
        ...
        if((NumberAcc<17)||
          ...
          delay(DelayProg);
            Serial.println(F("What is the operation mode? (1-8).")); delay(DelayWrite);
...
void ExecMMcommand() {
  if(TaskRGcommand==TaskRGcommandMem && PortAddress==NumberAcc && (millis()-100)<TimeCommand) {}
  else {
  NumberAcc=PortAddress;
